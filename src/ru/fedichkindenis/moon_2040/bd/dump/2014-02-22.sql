...
CREATE TABLE IF NOT EXISTS `game_statistics` (
  ...
  CONSTRAINT `fk$game_stat$res_stat$res_stat` FOREIGN KEY (`res_statistics`) REFERENCES `resources_statistics` (`id`)
...
CREATE TABLE IF NOT EXISTS `goal_game` (
  ...
  CONSTRAINT `fk$goal_game&func&func` FOREIGN KEY (`type_function`) REFERENCES `type_function` (`id`)
...
INSERT INTO `goal_game` (`game`, `resources`, `type_function`, `value_func`, `win`, `id`) VALUES
	(1, 1, 1, 0, 0, 1),
	(1, 1, 1, 100, 1, 2);
...
) ENGINE=InnoDB AUTO_INCREMENT=21 DEFAULT CHARSET=utf8 COMMENT='Состояние колонистов на каждом ходе игры';
